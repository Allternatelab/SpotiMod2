...
$url = "C:\ALLTERNATE\SPOTIFYMOD\patches/patches.json"
$retries = 0
...
if ($test_js) {
    ...
    if ($ch -eq 'y') { 
        $Url = "https://www.youtube.com/channel/UCL8ssJ6dcCofNZvWowEzLRQ"
        Start-Process $Url
    }
...
If ($test_spa) {
    ...
    If ($patched_by_spotx -match 'patched by Allternate lab') {
        $zip.Dispose()    
